...
class Analysis(threading.Thread):
  def __init__(self,sleeptime=120,verbose=False,stories_col="stories",tweet_col="tweets"):
  ...
  def run(self):
    while 1:
      #Sleep for sleeptime
      if self.verbose:
        print unicode("[INFO] Analysis Thread: sleeping for {0}.").format(self.sleeptime)
  ...
  def add_stories_to_mongo(self, stories):
    ...
    if self.verbose:
      print unicode("[INFO] Analysis Thread: Adding extra stories to db.")
      
    for story in stories:
      in_db = self.stories_collection.find_one({"title": story["title"]})
      if not in_db:
        if self.verbose:
          print unicode("[INFO] Analysis Thread: Story {0} is not in db. Adding...").format(story["title"])
        story["update_time"] = time.time()
        story["created_at"] = time.time()
        self.stories_collection.insert(story)
      else:
        if self.verbose:
          print unicode("[INFO] Analysis Thread: Story {0} is in db. Updating date.").format(story["title"])
        in_db.update({"$set": {"date": story["date"]}})
        if self.verbose:
          print unicode("[INFO] Analysis Thread: Story {0} is in db. Updating update_time.").format(story["title"])
        in_db.update({"$set": {"update_time": time.time()}})
        if self.verbose:
          print unicode("[INFO] Analysis Thread: Story {0} is in db. Updating keywords.").format(story["title"])
        in_db.update({"$set": {"keywords": story["keywords"]}})
        if self.verbose:
          print unicode("[INFO] Analysis Thread: Story {0} is in db. Updating link.").format(story["title"])
        in_db.update({"$set": {"link": story["link"]}})
        if self.verbose:
          print unicode("[INFO] Analysis Thread: Story {0} is in db. Updating main story link.").format(story["title"])
  ...
  def add_new_time_period_to_stories(self, stories, start, end):
    ...
    if self.verbose:
      print unicode("[INFO] Analysis Thread: Adding time period from {0} to {1} to stories.").format(start,end)
      
    #Add time period stub to stories
    if self.verbose:
      print unicode("[INFO] Analysis Thread: Adding curr_period to story objects")
    for story in stories:
    ...
    if self.verbose:
      print unicode("[INFO] Analysis Thread: Pushing new periods to db.")
      
    for story in stories:
      story["curr_period"]["wordstats"] = self.word_stats.get_word_statistics_for_tweets(story["curr_period"]["tweets"])
      if self.verbose:
        print unicode("[INFO] Analysis Thread: Pushing period with {0} tweets to {1}").format(len(story["curr_period"]["tweets"]), story["title"])
